...
namespace Vcsos.mm
{
    public class vminc : vmoperator
    {
        ...
        public bool ParseAndRun(ParserFactory factory)
        {
            ...
            {
                MemoryMap.Write(VM.Instance.MasterCore.Akku.Inc(MemoryMap.Read32(param1V)), (uint)param1V);
            }
            else if (param1 == InstructionParam2.Register)
            {
                VM.Instance.MasterCore.Register.Set(factory.m_pRegisters[param1V].Name,
                  VM.Instance.MasterCore.Akku.Inc(VM.Instance.MasterCore.Register.Get(factory.m_pRegisters[param1V].Name)));
            }
