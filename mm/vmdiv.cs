...
namespace Vcsos.mm
{
	public class vmdiv : vmoperator
	{
        ...
        public string Info
        {
            get { return "Div Number, Register, Pointer to AX - DIV #d5"; }
        }
        public bool ParseAndRun (ParserFactory factory)
		{
			...
			if (param1 == InstructionParam2.Value)
				VM.Instance.MasterCore.Akku.Div (param1V);
			else if (param1 == InstructionParam2.Register) {
				VM.Instance.MasterCore.Akku.Div (VM.Instance.MasterCore.Register.Get (factory.m_pRegisters [param1V].Name));
			}
			else if (param1 == InstructionParam2.Pointer)
				VM.Instance.MasterCore.Akku.Div (MemoryMap.Read32 (param1V));
