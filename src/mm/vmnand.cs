...
namespace Vcsos.mm
{
	public class vmnand: vmoperator
	{
		public string Name {
			get { return "NAND"; }
		}
        public string Info
        {
            get { return "Not And Operator2 Operator3 to Operator1 - NAND @d255, #d5, #d5 "; }
        }
        public bool ParseAndRun (ParserFactory factory)
		{
			...
			if (param3 == InstructionParam2.Value)
				VM.Instance.CurrentCore.Register.Stack.Push32 (param3V);
			else if (param3 == InstructionParam2.Register) {
				VM.Instance.CurrentCore.Register.Stack.Push32 (VM.Instance.CurrentCore.Register.Get (factory.m_pRegisters [param3V].Name));
			}
			else if (param3 == InstructionParam2.Pointer)
				VM.Instance.CurrentCore.Register.Stack.Push32 (MemoryMap.Read32 (param3V));
			///
			if (param1 == InstructionParam2.Pointer)
				MemoryMap.Write (VM.Instance.CurrentCore.Nand( VM.Instance.CurrentCore.Register.Stack.Pop32 (), VM.Instance.CurrentCore.Register.Stack.Pop32 () ), (uint)param1V);
			else if (param1 == InstructionParam2.Register)
				VM.Instance.CurrentCore.Register.Set (factory.m_pRegisters [param1V].Name, VM.Instance.CurrentCore.Nand( VM.Instance.CurrentCore.Register.Stack.Pop32 (), VM.Instance.CurrentCore.Register.Stack.Pop32 () ));
