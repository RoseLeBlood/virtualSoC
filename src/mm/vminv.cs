...
namespace Vcsos.mm
{
    public class vminv : vmoperator
    {
        ...
        public string Info
        {
            get { return "Invetiere ein Pointer oder Register - Inv @d255"; }
        }
        public bool ParseAndRun(ParserFactory factory)
        {
            ...
            {
                MemoryMap.Write(~(MemoryMap.Read32(param1V)), (uint)param1V);
            }
            else if (param1 == InstructionParam2.Register)
            {
                VM.Instance.CurrentCore.Register.Set(factory.m_pRegisters[param1V].Name,
                    ~(VM.Instance.CurrentCore.Register.Get(factory.m_pRegisters[param1V].Name)));
            }
